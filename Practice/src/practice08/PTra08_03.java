...
public class PTra08_03 {
	public static void main(String[] args) {
		...
		if (result) {//条件がtrueじゃないとダメ
			System.out.println("偶数です");
		} else {
			System.out.println("奇数です");//trueだからfalseは奇数
		}                                   //falseが返ってきた時点でelse
	 ...
	 */
	public static boolean isEvenNumber(int x) {//numberは数字だからint
		                                         //中身はランダムだからｘ

		if( x % 2 == 0) {  //2で割り切れたら
			return true;  //falseなら実行されないから
		}                  
		return false;     //そのままfalseを返す
